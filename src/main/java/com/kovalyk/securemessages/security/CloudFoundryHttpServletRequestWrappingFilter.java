...
public class CloudFoundryHttpServletRequestWrappingFilter extends
		GenericFilterBean {

	@Override
	public void doFilter(ServletRequest request, ServletResponse response,
			FilterChain chain) throws IOException, ServletException {
		chain.doFilter(new CloudFoundryHttpServletRequestWrapper(
				(HttpServletRequest) request), response);
	}

	static class CloudFoundryHttpServletRequestWrapper extends
			HttpServletRequestWrapper {

		private static final String[] TLS_HEADERS = new String[] {
				"sslclientcipher", "sslclientcertstatus" };

		public CloudFoundryHttpServletRequestWrapper(
			...
			if (!isSecure) {
				...
				for (String header : TLS_HEADERS) {
					isActuallySecure = isActuallySecure
							&& StringUtils.isNotBlank(getHeader(header));
				}
